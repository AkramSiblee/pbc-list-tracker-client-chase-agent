...
2. **Confidentiality is a hard gate, not a chase parameter.** Restricted-tier
   items (`config/confidentiality_rules.json`) never enter the automated
...
- **Config is generated, not hand-edited.** `config/*.json` is produced by
  `scripts/generate_config.py` from `sample_data/engagement_setup/
...
- **`output/` is scratch space**, not committed deliverables — tracker
  mirrors, classification results, and extracted-item dumps land here and are
  regenerated by re-running the relevant script.
